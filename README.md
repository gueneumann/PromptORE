# GN Version of Unsupervised Relation Extraction
...
Although recent approaches achieve promising results, they heavily depend on hyperparameters whose tuning would most often require labeled data. To mitigate the reliance on hyperparameters, we propose **GNORE**, a two-stage approach for unsupervised relation extraction: step 1) use LLM to infer relation embedding for given relation instances, 2) perform clustering for proposing relation groups.

Results on several general and specific domain datasets show that **GNORE** performs state-of-the-art on standard measures: B3, V-measure and ARI. We perform experiments with masked language models and clausal language models and several other features.
...
`experiments/run_experiments.py` runs a batch of GNORE configurations (different datasets, models,
...
The source code of GNORE is licensed under the GPLv3 License. For more details, please refer to the [LICENSE.md file](LICENSE.md).
